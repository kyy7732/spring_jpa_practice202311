...
class DepartmentRepositoryTest {
    ...
    @Test
    @DisplayName("양방향 연관관계에서 연관데이터의 수정") // 양방향 매핑인 경우 최신업데이트를 진행해 줘야 한다.
    void testChangeDept() {
        ...
        // 1. -> 변경 감지(더티 체크) 후 변경된 내용을 DB에 즉시 반영하는 역할.
...
//        entityManager.clear(); // 영속성 컨텍스트 비우기 (비우지 않으면 컨텍스트 내의 정보를 참조하려 함.)
    ...
    void testNPlus1Solution() {
        //given
        List<Department> departments = departmentRepository.findAllIncludesEmployees(); // 우리가 직접 작성한 jpql로 바꿔줌
        //when
