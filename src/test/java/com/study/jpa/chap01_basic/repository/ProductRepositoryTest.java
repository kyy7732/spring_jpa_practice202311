...
@Rollback(false) // false로 두면 롤백을 실행하지 않는다.
class ProductRepositoryTest {
    ...
    void insertDummyDate() {
        ...
        Product p4 = Product.builder()
                ...
                .build();
        //when
        productRepository.save(p1); // .save(): 상속받은 JpaRepository가 주는 함수
    ...
    @Test
    @DisplayName("5 번째 상품을 데이터베이스에 저장해야 한다.")
    void testSave() {
        //given
    ...
    void testFindOne() {
        ...
        //then
        product.ifPresent(p -> {
            assertEquals("구두", p.getName()); // p.getName이 구두다
        });
    ...
    void testModify() {
        ...
        Optional<Product> product = productRepository.findById(id);
        product.ifPresent(p -> { // product가 존재한다면
            p.setName(newName);
            p.setPrice(newPrice);

            productRepository.save(p); // setter를 부르고 나서 save를 부르기때문에 update에서 사용된다
        });
