...
public interface StudentRepository extends JpaRepository<Student, String> {
    ...
    List<Student> findByName(String name); // 반드시 필드명과 동일하게 이름 생성

    List<Student> findByCityAndMajor(String city, String major); // where절에 City AND Major
...
//    @Query(value = "SELECT * FROM tbl_student WHERE city = ?1", nativeQuery = true)
        ...
        @Query("SELECT s FROM Student s WHERE s.name LIKE %:nm%") // 이렇게 쓰는걸 선호한다
        List<Student> searchByNameWithJPQL(@Param("nm") String name);

        // JPQL로 수정 삭제 쿼리 쓰기
        @Modifying // JPQL에서 조회가 아닐 경우 무조건 붙여야 함.
